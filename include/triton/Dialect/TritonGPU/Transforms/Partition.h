...
public:
  ...
  SmallVector<std::unique_ptr<Partition>> takePartitions() {
...
// Compatibility wrapper for legacy HCU warp specialization code.
class WarpSchedule : public PartitionSet {
public:
  using PartitionSet::addPartition;
  using PartitionSet::getNumPartitions;
  using PartitionSet::getPartition;
  using PartitionSet::getPartitions;
  using PartitionSet::getTag;

  Partition *getRootPartition();
  const Partition *getRootPartition() const;
  ...
  static void eraseFrom(Operation *op);
};
