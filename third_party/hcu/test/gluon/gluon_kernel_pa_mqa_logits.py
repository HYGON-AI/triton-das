...
def _run_pa_mqa_logits(
    ...
    _, kv_block_size, _, index_dim = kv_cache.size()

    assert index_dim == hidden_dim + 4
    if q_fp8.dtype != _FP8_DTYPE:
        q_fp8 = q_fp8.to(_FP8_DTYPE)

    tile_q_count = max(1, batch_size * next_n)
    total_cu = torch.cuda.get_device_properties(torch.cuda.current_device()).multi_processor_count
    wave_per_eu = 2
    split_kv = (max(1, total_cu // tile_q_count) + 4) // 5 * 5 * wave_per_eu

    kv_flat = kv_cache.view(-1, kv_block_size * index_dim)
    kv_cache_fp8 = kv_flat[..., : kv_block_size * hidden_dim].view(_FP8_DTYPE)
    kv_cache_scale = kv_flat[..., kv_block_size * hidden_dim :].view(torch.float32)

    grid = (batch_size * next_n * split_kv, 1, 1)
...
def _decode_packed_kv_cache(kv_cache_fp8: torch.Tensor, hidden_dim: int, block_size: int):
    ...
    keys = kv_fp8.view(_FP8_DTYPE).float() * kv_scale
    return keys
...
def test_gluon_pa_mqa_logits_reference_small():
    arch = _get_arch()
    if arch not in ("gfx938", "gfx942", "gfx950"):
        pytest.skip(f"gluon_kernel_pa_mqa_logits only supports gfx938/gfx942/gfx950, got {arch}")

    torch.manual_seed(0)
    batch_size, next_n, heads, hidden_dim = 1, 1, 64, 128
    max_model_len = 64
    context_lens_list = [59]

    block_size = 1
    num_blocks = max_model_len

    q = torch.randn((batch_size, next_n, heads, hidden_dim), device="cuda", dtype=torch.bfloat16)
    q_fp8 = q.to(_FP8_DTYPE)
