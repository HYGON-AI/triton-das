...
_LOGGER = logging.getLogger("hcu.gluon.gemm_a8w8")
...
def _resolve_gluon_arch(arch: str) -> str:
    # Keep kernel code unchanged; only reuse gfx950 configs for gfx938 validation.
    return "gfx950" if arch == "gfx938" else arch
...
try:
    ...
    _USE_2D_MFMA_INSTR_SHAPE = tl.constexpr(True)
except Exception:
    _USE_2D_MFMA_INSTR_SHAPE = tl.constexpr(False)
...
def _gemm_a8w8_kernel(
    ...
    mfma_layout: gl.constexpr = gl.amd.AMDMFMALayout(
        version=4,
        instr_shape=[16, 16] if _USE_2D_MFMA_INSTR_SHAPE else [16, 16, 32],
        transposed=True,
...
def _get_config(
    ...
    dev = _resolve_gluon_arch(_get_arch())
    if dev != "gfx950":
        raise ValueError(
            f"Gluon GEMM-A8W8 only supports gfx938/gfx950 in this test, got {dev}."
        )
    return _DEFAULT_GEMM_CONFIG.copy()
...
def test_gluon_gemm_a8w8_reference_small():
    arch = _get_arch()
    if arch not in ("gfx938", "gfx950"):
        pytest.skip(f"gluon_kernel_gemm_a8w8 only supports gfx938/gfx950, got {arch}")
    ...
    x = torch.randint(-127, 127, (m, k), device="cuda", dtype=torch.int8)
    w = torch.randint(-127, 127, (n, k), device="cuda", dtype=torch.int8)
    x_scale = torch.rand((m,), device="cuda", dtype=torch.float32) * 0.02 + 0.001
    ...
    ref = (x.float() * x_scale[:, None]) @ (w.float() * w_scale[:, None]).T
    torch.testing.assert_close(y.float(), ref, rtol=2e-2, atol=2e-1)
