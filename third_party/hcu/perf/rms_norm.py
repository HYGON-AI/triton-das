...
class _RMSNormFunction(torch.autograd.Function):
    ...
    def forward(ctx, X, W, eps, offset=0.0, out_dtype=torch.float16):
        ...
        _rms_norm_fwd_rocm[grid](
            ...
            USE_BLOCKED,
            NUM_PRGMS)
        # liger-kernel implement
        # if n_cols > BLOCK_SIZE:
        #     raise RuntimeError("This layer norm doesn't support feature dim >= 64KB.")
        # _rms_norm_fwd_liger[(n_rows,)](
        #     Y,
        #     Y.stride(0),
        #     X,
        #     X.stride(0),
...
def run_benchmark(args):
    ...
    def benchmark(M, N, provider, mode):
        ...
        offset = 0.0
        if provider == 'torch':
            fn = lambda: torch_rmsnorm(x, w, esp, offset, dtype)
            if mode == 'bwd':
                dy = torch.randn_like(x)
                y = fn()
                fn = lambda: y.backward(dy)
