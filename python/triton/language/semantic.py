...
# ===----------------------------------------------------------------------===//
    ...
    def matrix_load(self,
        ...
        result = self.tensor(self.builder.create_matrix_load(base.handle, [s.handle for s in shape],
        ...
        if mask is not None:
            mask = self.broadcast_impl_shape(mask, block_shape)
            # Create zero tensor with same shape and dtype as result
            zero_value = self.builder.get_null_value(base.type.element_ty.to_ir(self.builder))
            zero_tensor = self.tensor(zero_value, base.type.element_ty)
            zero_result = self.splat(zero_tensor, block_shape)
            # Apply mask: where(mask, result, zero_result)
            result = self.where(mask, result, zero_result)
