...
def persistent_all_reduce(
    ...
    # get the ptr of the next rank
    next_flag_ptr = libshmem_device.remote_ptr(flags, next_rank)
    next_ring_buffer_ptr = libshmem_device.remote_ptr(ring_buffer, next_rank)
    ...
    for g in range(pid, num_groups, COMM_SMS):
        ...
        for s in range(0, group_size):
            ...
            if s == 0:
                ...
                while tl.atomic_cas(locks + idx, 1, 0, sem="acquire", scope="gpu") != 1:
                    pass
                ...
                if group_size > 1:
                    ...
                    # while iris.atomic_cas(flags + idx, 0, 0, cur_rank, next_rank, heap_bases, sem="acquire", scope="sys") != 0:
                    #     pass
                    # if cur_rank == 0 and pid == 0 and tid == 0:
                    #     a  = tl.load(next_flag_ptr + idx)
                    #     print("wait", a)
                    while tl.atomic_cas(next_flag_ptr + idx, 0, 0, sem="acquire", scope="sys") != 0:
                        pass
                    ...
                    tl.store(next_ring_buffer_ptr + goff, acc, mask=sub_mask)
                    # iris.atomic_xchg(flags + idx, 1, cur_rank, next_rank, heap_bases, sem="release", scope="sys")
                    tl.atomic_xchg(next_flag_ptr + idx, 1, sem="release", scope="sys")
            else:
                # Receive the traveling accumulator for this tile from PREV
                while tl.atomic_cas(flags + idx, 1, 0, sem="acquire", scope="sys") != 1:
                    pass
                ...
                if s < group_size - 1:
                    ...
                    for rank in range(world_size):
                        if rank == cur_rank:
                            ...
                            remote_c_ptr = libshmem_device.remote_ptr(C, rank)
                            tl.store(remote_c_ptr + goff, c, mask=sub_mask)
...
def triton_gemm_allreduce(
    a, b,
    output,           # final output
    scatter_buf,     # gemm bufs
    scatter_signal_buf,
    ring_buf,        # ring bufs
    ring_flag,       # ring flags
    sync_bufs_ptr,
...
if __name__ == "__main__":
    ...
    if args["check"]:
        ...
        # print(f"RANK {RANK}: output {dist_out_list[0]}")
