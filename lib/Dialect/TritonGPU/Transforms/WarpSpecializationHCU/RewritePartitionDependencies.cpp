...
struct AsyncRef {
  ...
  auto getBarIds(ImplicitLocOpBuilder &b, Value idx) const {
      auto intCst = [&](int value) { return b.create<arith::ConstantIntOp>(value, 32); };
      Value readyBarId = intCst(readyBarIds.back());
      for (int i = readyBarIds.size() - 2; i >= 0; --i) {
        Value cond = b.create<arith::CmpIOp>(arith::CmpIPredicate::eq, idx, intCst(i));
        readyBarId = b.create<arith::SelectOp>(cond, intCst(readyBarIds[i]), readyBarId);
      }
      Value emptyBarId = intCst(emptyBarIds.back());
      for (int i = emptyBarIds.size() - 2; i >= 0; --i) {
        Value cond = b.create<arith::CmpIOp>(arith::CmpIPredicate::eq, idx, intCst(i));
        emptyBarId = b.create<arith::SelectOp>(cond, intCst(emptyBarIds[i]), emptyBarId);
      }
      return std::make_tuple(readyBarId, emptyBarId);
  }
  unsigned multiplicitySize;
  unsigned maxDistance;
  Value alloc;
  SmallVector<int> readyBarIds;
  SmallVector<int> emptyBarIds;
...
AsyncRef DependencyRewriter::allocateAsyncValue(RankedTensorType tensorType,
  ...
  SmallVector<int> readyBarIds = getAvailableAbarrierIds(loop, numBars);
  SmallVector<int> emptyBarIds = getAvailableAbarrierIds(loop, numBars);
...
void DependencyRewriter::initializeBarriers(int index, const AsyncRef &aref,
  ...
  auto readyBarId = aref.readyBarIds[index];
  auto emptyBarId = aref.emptyBarIds[index];
  b.create<ROCDL::HCUAbarrierInitOp>(readyBarId, 1);
  b.create<ROCDL::HCUAbarrierInitOp>(emptyBarId, numConsumers);
  if (init)
    b.create<ROCDL::HCUAbarrierArriveOp>(readyBarId, 1);
  else
    b.create<ROCDL::HCUAbarrierArriveOp>(emptyBarId, numConsumers);

  endBuilder.create<ROCDL::HCUAbarrierInvOp>(readyBarId);
  endBuilder.create<ROCDL::HCUAbarrierInvOp>(emptyBarId);
}
  ...
  for (auto [partition, useInfo] :
    ...
    for (auto &[output, info] : useInfo) {
      ...
      for (auto &[key, uses] : info.consumers) {
        ...
        auto view = aref.getValueView(b, idx);
        auto [readyBarId, emptyBarId] = aref.getBarIds(b, idx);
        StageCluster sinkSrcCluster = getStageCluster(earliestUser);
        b.createInto<ROCDL::HCUAbarrierTryWaitOp>(*usePartition, sinkSrcCluster,
                                                  readyBarId, phase);
        ...
        b.createInto<ROCDL::HCUAbarrierArriveOp>(*usePartition, sinkSrcCluster, emptyBarId, 1);
      }
      ...
      b.createInto<ROCDL::HCUAbarrierTryWaitOp>(partition, srcStageCluster, emptyBarId, phase);
      b.createInto<LocalStoreOp>(partition, srcStageCluster, output, view);
      b.createInto<ROCDL::HCUAbarrierArriveOp>(partition, srcStageCluster, readyBarId, 1);
    }
  }
...
#define GEN_PASS_DEF_TRITONGPUREWRITEPARTITIONDEPENDENCIESHCU
#include "triton/Dialect/TritonGPU/Transforms/Passes.h.inc"
} // namespace mlir::triton::gpu

namespace {
struct RewritePartitionDependencies
    : triton::gpu::impl::TritonGPURewritePartitionDependenciesHCUBase<
          RewritePartitionDependencies> {
  using TritonGPURewritePartitionDependenciesHCUBase::
      TritonGPURewritePartitionDependenciesHCUBase;
...
void RewritePartitionDependencies::runOnOperation() {
  ...
  for (scf::ForOp loop : loops) {
    if (failed(mlir::triton::gpu::hcu::rewritePartitionDependencies(loop)))
