...
// RUN: triton-opt %s -split-input-file --tritonhcu-update-async-wait-count=arch-generation-name=gfx938 | FileCheck %s --check-prefix=ASYNCMARK
// RUN: triton-opt %s -split-input-file --tritonhcu-update-async-wait-count=arch-generation-name=gfx90a | FileCheck %s --check-prefix=LEGACY

// MatrixLoadToLocal participates in the same async commit/wait accounting as
// AsyncCopyGlobalToLocal. On asyncmark HCU targets, preserve commit-group
// counts for LLVM. The legacy path counts the generated matrix instructions.
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "hip:gfx938", "ttg.threads-per-warp" = 64 : i32} {
  // ASYNCMARK-LABEL: matrix_load_to_local_waitcnt
  // LEGACY-LABEL: matrix_load_to_local_waitcnt
  tt.func public @matrix_load_to_local_waitcnt(
    ...
    // ASYNCMARK: ttg.async_wait {{.*}} {num = 1 : i32}
    // ASYNCMARK-NOT: amdg.async_wait
    // LEGACY: amdg.async_wait {{.*}} {num_inst = 8
    %4 = ttg.async_wait %1 {num = 1 : i32}
    // ASYNCMARK: ttg.async_wait {{.*}} {num = 0 : i32}
    // LEGACY: amdg.async_wait {{.*}} {num_inst = 0
