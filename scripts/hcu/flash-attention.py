...
@triton.autotune(
   configs=[
       ...
       triton.Config({'BLOCK_M': 32, 'BLOCK_N': 16, 'waves_per_eu': 1, 'PRE_LOAD_V': False}, num_stages=1, num_warps=1),
       triton.Config({'BLOCK_M': 64, 'BLOCK_N': 32, 'waves_per_eu': 1, 'PRE_LOAD_V': False}, num_stages=1, num_warps=1),
...
def nonvarlen_benchmark_configs():
    ...
    # config sizes from Ali test cases
    configs=[[1, 16, 16, 8192, 8192],
            [1, 32, 32, 8192, 8192],
            [1, 8, 1, 8192, 8192],
            [1, 13, 1, 8192, 8192],
            [1, 16, 2, 8192, 8192],
...
def run_benchmark(custom):
    ...
    for mode in args.mode.split(','):
        # Bwd pass only supports causal=True and MHA right now
        if mode == 'bwd':
            causal = True
            # MHA, HK = HQ
        ...
        configs.append(triton.testing.Benchmark(
            ...
            styles=[('red', '-')],
            ylabel='ms',
            plot_name=f'fused-attention-{mode}-d{head_size}{"-varlen" if varlen else ""}{"-causal" if causal else ""}',
            args={
                'D_HEAD': head_size,
                'dtype': dtype,
                'causal': causal,
